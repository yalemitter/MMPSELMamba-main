...
parser.add_argument('--dataset', '-d', metavar='NAME', default='cifair',
...
parser.add_argument('--opt-eps', default=.001, type=float, metavar='EPSILON',
...
parser.add_argument('--lr', type=float, default=0.064, metavar='LR',
                    help='learning rate (default: 0.05)')
parser.add_argument('--lr-noise', type=float, nargs='+', default=0.42, metavar='pct, pct',
...
parser.add_argument('--decay-rate', '--dr', type=float, default=.973, metavar='RATE',
...
parser.add_argument('--output', default='./output+dir/wave', type=str, metavar='PATH',
...
def main():
    ...
    print(f'Total number of model0 parameters: {total_params}')
    ...
    def count_params_by_type(model):
        ...
        for name, param in model.named_parameters():
            if 'weight' in name:
                if 'conv' in name:
                    counts['conv'] += param.numel()
                elif 'linear' in name or 'fc' in name:
                    counts['linear'] += param.numel()
                elif 'norm' in name or 'bn' in name:
                    counts['norm'] += param.numel()
                elif 'embed' in name:
        ...
        return counts
    print("counts:",count_params_by_type(model))
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[args.local_rank], find_unused_parameters=True, broadcast_buffers=not args.no_ddp_bb)
